...
macro_rules! new_full_start {
	($config:expr) => {{
		...
		let builder = substrate_service::ServiceBuilder::new_full::<
			node_primitives::Block, node_runtime::RuntimeApi, node_executor::Executor, im_online::ed25519::AuthorityId
		>($config)?
...
-> Result<impl AbstractService, ServiceError> {
	...
	let service = ServiceBuilder::new_light::<Block, RuntimeApi, node_executor::Executor, im_online::ed25519::AuthorityId>(config)?
		.with_select_chain(|_config, client| {
			#[allow(deprecated)]
